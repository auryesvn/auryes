...
export function UniverseHome({ basePath }: { basePath: NamespaceBasePath }) {
  ...
  return <main data-tinh-ma-world className="experience is-universe catalogue-home">
    <header className="topbar">
      <Link className="brand" href={homeHref} aria-label="Trang chủ 3288">3288</Link>
    ...
    <section id="catalogue" className="universe catalogue" aria-label="Danh mục 18 thế giới âm nhạc">
      ...
      <aside className="focus-card catalogue-focus visible" aria-live="polite">
        ...
        <div><p className="eyebrow">{focused.status === "live" ? "THẾ GIỚI ĐANG MỞ" : "ĐANG HÌNH THÀNH"}</p><h1>{focused.title}</h1>{focused.route ? <Link className="catalogue-enter" href={`${basePath}${focused.route}`}>ĐI VÀO THẾ GIỚI <span>→</span></Link> : <p className="catalogue-forming">Thế giới này đang hình thành.</p>}</div>
      </aside>
